...
class Promo extends Sistema {
    ...
    function update($id, $data) {
        ...
        if ($_FILES['fotografia']['error'] != 4) {
            $fotografia = $this->uploadFoto();  // Subir la foto
            $tmp = "fotografia = :fotografia";  // Solo agrega el campo de fotografía si existe
        }
        ...
        // Agrega el campo de fotografía si se ha subido un archivo
        if ($tmp !== "") {
            $sql .= ", " . $tmp;
        }
        
        $sql .= ' WHERE id_promo = :id_promo';  // Finaliza la consulta
    ...
    function uploadFoto() {
        ...
        return $default;  // Si no se subió archivo, devuelve el valor por defecto
    }
}
?>
