...
<div class="mx-3 my-5">
    <form action="promo.php?accion=<?php if ($accion == "crear"): echo('nuevo'); else: echo('modificar&id=' . $id); endif; ?>" method="post" enctype="multipart/form-data"> 
        ...
        <!-- Campo Fecha Descripcion -->
        <div class="row mb-3">
            <label for="rfc" class="col-sm-2 col-form-label">Descripcion</label>
            <div class="col-sm-10">
                <input type="text" name="data[descripcion]" placeholder="Escribe una descripcion" class="form-control" value="<?php if(isset($promos['descripcion'])):echo($promos['descripcion']);endif; ?>"/>
            </div>
        </div>
        <!-- Campo Fotografia -->
        <div class="row mb-3">
